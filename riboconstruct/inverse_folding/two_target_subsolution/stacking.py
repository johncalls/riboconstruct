...
class TwoTargetStackSubsolution(subsolution.TwoTargetSubsolution):
    def __init__(self, struct_id, substruct, task, predecessor):
        super(TwoTargetStackSubsolution, self).__init__(
            struct_id, substruct, task, predecessor)

        self.new_filling = fl.TwoTargetStackFilling
    ...
    def append_min_filling(self, filling):
        ...
        super(TwoTargetStackSubsolution, self).append_min_filling(filling)

    def append(self, filling):
        if self.filling_is_constrained(filling):
            return

        super(TwoTargetStackSubsolution, self).append(filling)
